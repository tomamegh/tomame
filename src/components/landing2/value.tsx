          ...
          return (
            ...
            >
              ...
              <div className="flex-1 space-y-6">
                <span className="inline-block px-3 py-1 rounded-full bg-primary/20 text-primary text-xs font-semibold tracking-wide uppercase">
                  {vp.badge}
                </span>
                ...
                <ul className="space-y-3">
                  {vp.bullets.map((b) => (
                    <li key={b} className="flex items-start gap-3">
                      <div className="mt-1 w-5 h-5 rounded-full bg-linear-to-br from-primary to-teal-500 flex items-center justify-center shrink-0">
                ...
                <Link href="/auth/signup">
                  <Button className="gradient-secondary border-none" size={'lg'}>Get Started</Button>
                </Link>
              </div>

              {/* Illustration card */}
              <div className="flex-1 w-full">
                <div className="rounded-2xl sm:rounded-3xl gradient-secondary p-5 sm:p-8 shadow-[0_16px_48px_-8px_rgba(0,0,0,0.2)]">
                  <div className="space-y-4">
                    {i === 0 ? (
                      /* Product extraction mockup */
                      <>
                        <div className="flex items-center gap-3 rounded-xl bg-white/25 border border-white/45 px-4 py-3">
                          <div className="w-8 h-8 rounded-lg bg-white flex items-center justify-center text-sm">🔗</div>
                          <div className="flex-1 rounded text-white">https://amazon.com/dp...</div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {['Product Name', 'Price', 'Weight', 'Platform'].map((l) => (
                            <div key={l} className="rounded-xl bg-white/25 border border-white/5 p-3">
                              <p className="text-[10px] text-white/80 mb-1">{l}</p>
                              <div className="h-3 w-3/4 rounded bg-white/20" />
                            </div>
                          ))}
                        </div>
                        <div className="rounded-xl bg-white border border-emerald-500/20 px-4 py-3 flex items-center gap-2">
                          <span className="text-teal-500 text-xs">✓</span>
                          <span className="text-teal-500 text-xs">Product data extracted successfully</span>
                        </div>
                      </>
                    ) : (
                      ...
                      <>
                        <div className="flex items-center justify-between mb-2">
                          <p className="text-white/80 text-xs">Orders this month</p>
                        ...
                        <div className="h-24 sm:h-32 flex items-end gap-1.5">
                          {[30, 50, 40, 70, 55, 85, 65, 90, 75, 95, 80, 100].map((h, j) => (
                            <div
                              key={j}
                              className="flex-1 rounded-t bg-linear-to-t from-white/50 to-white/30"
                              style={{ height: `${h}%` }}
                            />
                          ))}
                        </div>
                        <div className="grid grid-cols-3 gap-3 mt-4">
                          {[
                            ...
                            <div key={s.label} className="rounded-xl bg-white/3 border border-white/5 p-3 text-center">
                              <p className="text-[10px] text-white/80 mb-1">{s.label}</p>
