    ...
    return (
        <section className="py-24 sm:py-32">
            <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                >
                    <motion.p variants={fadeUp} className="text-sm font-semibold mb-3 tracking-wide uppercase">
                        Testimonials
                    </motion.p>
                    <motion.h2 variants={fadeUp} className="text-3xl sm:text-4xl font-bold text-stone-850 mb-12">
                ...
                <div className="relative overflow-hidden">
                    <AnimatePresence mode="wait">
                        <motion.div
                            ...
                            className="rounded-2xl border border-purple-500/10 bg-purple-950/30 backdrop-blur-sm p-8 sm:p-10 text-center relative overflow-hidden"
                        >
                            ...
                            <div className="flex items-center justify-center gap-4">
                                <div className="w-12 h-12 rounded-full bg-linear-to-br from-violet-500 to-pink-500 flex items-center justify-center text-white text-sm font-bold shadow-[0_0_20px_-4px_rgba(168,85,247,0.4)]">
                                    {TESTIMONIALS[active]!.initials}
                                </div>
                                <div className="text-left">
                                    <p className="font-semibold text-white">{TESTIMONIALS[active]!.name}</p>
                                    <p className="text-purple-400/50 text-sm">{TESTIMONIALS[active]!.role}</p>
                                </div>
                            </div>
                        </motion.div>
                    </AnimatePresence>
                ...
                <div className="flex justify-center gap-2 mt-8">
                    {TESTIMONIALS.map((_, i) => (
                        <button
                            key={i}
                            onClick={() => setActive(i)}
                            className={`w-2.5 h-2.5 rounded-full transition-all duration-300 cursor-pointer ${i === active ? 'bg-fuchsia-400 w-8' : 'bg-purple-700 hover:bg-purple-600'
                                }`}
                        />
                    ))}
                </div>
