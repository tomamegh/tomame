...
function NewOrderContent() {
  ...
  useEffect(() => {
    // The caller only renders this when a `url` is present; the guard stays as
    // belt and braces rather than as the screen's behaviour.
    if (!url) return;
...
/**
 * The `?url=` path: extract that one link and forward to its price.
 *
 * Kept because a link arriving from elsewhere — the Home paste bar, a shared
 * URL, the marketing site — should land on the price, not on a work list. The
 * screen WITHOUT a `?url=` is `PasteQueueView`, which is where the paste queue
 * lives; see `page.tsx`.
 */
